...
small_dat <- dplyr::select(
  bes, ukipvoter, income, women_equal, edlevel,
  edlevelF, j05
) %>% na.omit()
...
res2 <- optim(par = c(0, 0), fn = sum_sq_resids_vec, y = small_dat$ukipvoter, z = small_dat$income, control=list(trace=2) )
res2$par
## smaller than ssr=203
res2$value

res2a <- optim(par = c(100, 100), fn = sum_sq_resids_vec, y = small_dat$ukipvoter, z = small_dat$income,control=list(trace=2)) 
res2a$par
res2a$value
