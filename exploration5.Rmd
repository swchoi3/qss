...
surveydat %>%
  group_by(treat) %>%
  summarize(primary_schooling = mean(primary_schooling))
surveydat %>%
  group_by(treat) %>%
  summarize(secondary_schooling_or_more = mean(secondary_schooling_or_more))
surveydat %>%
  group_by(treat) %>%
  summarize(reg_income = mean(reg_income))
surveydat %>%
  group_by(treat) %>%
  summarize(member = mean(member))
```
```{r}
